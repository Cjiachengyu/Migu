...
<title>${viewModel.showGift.gift.sender.userName}送给了${viewModel.showGift.gift.receiver.userName}1个${viewModel.showGift.gift.prize.prizeName}！</title>
...
<div id="animation_img_holder" class="animation_img_holder">
    <img class="show_img" src="${resRoot}/image/user/show/gift/send/show_send_gift.png?version=${resVersion}">
    <div class="show_tip">${viewModel.showGift.shower.userName}：${viewModel.showGift.gift.sender.userName}
	    <c:if test="${viewModel.showGift.gift.receiver == null}">送出了</c:if>
	    <c:if test="${viewModel.showGift.gift.receiver != null}">送给了${viewModel.showGift.gift.receiver.userName}</c:if>
	    1个${viewModel.showGift.gift.prize.prizeName}！
    </div>
