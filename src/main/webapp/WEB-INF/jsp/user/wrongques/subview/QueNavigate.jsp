...
<div class="que_num_nav_holder">
    ...
    <div class="que_num_count">第<span></span>题/共${viewModel.userAnswerList.size()}题</div>
    <div class="que_num_nav_icon que_num_nav_right_icon" onclick="gotoNext();"></div>
</div>

<!-- 题目区 -->
<div class="que_list_holder" id="queList">
    <c:forEach var="num" begin="1" end="${fn:length(viewModel.userAnswerList)}">
        ...
        <div class="que_holder" id="que_holder_${num}">
        	<div class="end_answer_time">收录时间:${viewModel.userAnswerList[num-1].endAnswerTimeString}</div>
            ...
            <c:if test="${isViewingAnalysis}">
            	<c:if test="${que.kaodian != null && que.kaodian != ''}">
                ...
                <c:if test="${que.analysisHtml != null && que.analysisHtml != ''}">
                    <div class="que_analysis_holder">
                        <div class="que_analysis_title_holder">分${que.kaodian}析：</div>
...
<div class="common_dock_bottom">
    <div class="operation_btn_holder">
        <a class="oper_que_nav_btn" onclick="gotoPrev();">上一题</a>
        <a class="oper_que_nav_btn" onclick="gotoNext();">下一题</a>
    </div>
</div>
...
<div id="user_answer_data_holder" class="hidden">
    ...
    <input id="que_list_count" type="hidden" value="${fn:length(viewModel.userAnswerList)}">
...
<c:if test="${isViewingAnalysis}">
    <script src='${resRoot}/js/user/wrongQues.js?version=${resVersion}' type='text/javascript'></script>
</c:if>
