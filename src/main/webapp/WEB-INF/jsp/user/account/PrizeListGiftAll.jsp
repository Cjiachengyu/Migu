...
<div class="sub_tab_holder clearfix">
    ...
    <a class="sub_tab sub_tab_selected">礼物</a>
</div>

<input type="hidden" id="myMoney" value="${viewModel.user.money}">

<div>
    <c:forEach items="${viewModel.giftList }" var="gift">
        <!-- 送出的礼物 -->
        <c:if test="${gift.senderId == viewModel.user.userId}">
        ...
        <!-- 收到的礼物 -->
        <c:if test="${gift.receiverId == viewModel.user.userId}">
            <div class="prize_holder">
                <div class="prize_image_holder">
                    <img class="prize_image" src="${gift.prize.prizeImageUrl}">
                </div>
